...
raising ET.ParseError (with a one-time warning) rather than reverting to
the vulnerable stdlib parser.
...
def _warn_missing_defusedxml_once() -> None:
    ...
    log.warning(
        'feedpakr_safe_xml: defusedxml not installed; refusing to parse '
        'untrusted XML (install defusedxml for hardened parsing)'
    )
...
def safe_parse(source: XMLSource, *args, **kwargs) -> ET.ElementTree:
    ...
    if not _HAVE_DEFUSEDXML:
        ...
        # Fails closed: falling back to stdlib here would re-open exactly
        # the entity-expansion hole this module exists to close.
        raise ET.ParseError(
            'defusedxml not installed; refusing to parse untrusted XML '
